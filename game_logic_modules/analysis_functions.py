...
def category_value(category, roll, state):
    """Function to calculate the total value of the category"""
...
def is_category_unavailable(category, roll, state):
    """Determines if a category is unavailable using joker rules"""
...
def roll_value(state, roll, free_categories, roll_num):
    """Recursively computes the value of a roll by considering all possible holds and categories."""
...
def calculate_choice(state, roll, free_categories, roll_num, points_away_from_optimal):
    """Calculates the value of each choice (category if roll_num == 2, else hold), and returns the choice which is the least valuable choice above the threshold."""
    values = {}
