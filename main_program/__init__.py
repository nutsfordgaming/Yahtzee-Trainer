...
def home(root):
    ...
    def render_difficulty(val):
        """Renders the new difficulty when the difficulty of the player is changed"""
    ...
    # Points away from optimal slider
...
def play(root, players):
    ...
    def move_to_keep(index):
        """Move a die from the roll area to the keep area."""
    ...
    def perform_choice(points_away_from_optimal, state, free_categories):
        """Handles when the suitable choice is wanted"""
    ...
    def render_players_score():
        """Calculate and display scores for the current players's column."""
    ...
    # Create a frame for the score card
    ...
    # Creates canvases for each dice in the keep area
    keep_dice_canvases = []
    for i in range(5):
